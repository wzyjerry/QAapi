...
def ner(txt, model, word_to_id, char_to_id, tag_to_id, id_to_tag):
    txt = txt.strip()
    word = []
    ...
    sentences = load_sentences(tmp_file[1], lower=False, zeros=False)

    input_data = prepare_dataset(
        sentences, word_to_id, char_to_id, tag_to_id, lower=False
    )
